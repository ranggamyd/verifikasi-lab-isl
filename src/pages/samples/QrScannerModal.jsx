...
const QRScannerModal = ({ show, onHide, fetchSamples }) => {
	...
	const handleInputChange = (e, koding) => {
		...
		setDataBottles((prevBottles) => prevBottles.map((bottle) => (bottle.koding === koding ? { ...bottle, jumlah: parseInt(value) || 0 } : bottle)));
	};

	const isReady = (bottle) => {
		return bottle.disiapkan && bottle.jumlah && bottle.disiapkan.toString() === bottle.jumlah.toString();
	};
	...
	const checkBottleData = async (value) => {
		...
		try {
			const response = await Api.fetchGet({ no_sampel: value }, makeSlice(controller, "checkBottle2"));
					...
					response.data_botol.forEach((newItem) => {
						...
						if (existingIndex !== -1) {
							...
							combined[existingIndex] = {
								...existing,
								...newItem,
								jumlah: existing.jumlah >= existing.disiapkan ? existing.disiapkan : (parseInt(existing.jumlah) || 0) + (parseInt(newItem.add) || 0),
							};
						} else {
		...
		<>
			<style jsx global>{`
				...
				.qr-scanner-region > div {
					border: 2px solid transparent !important;
					background: linear-gradient(135deg, #00ff87, #60efff); /* Hijau to Cyan */
					border-radius: 8px !important;
				}

				.bottle-ready {
					background: linear-gradient(135deg, #d4edda, #a8e6cf) !important; /* Hijau lembut */
					border-color: #88d4ab !important;
					color: #155724 !important;
				}

				.bottle-not-ready {
					background: linear-gradient(135deg, #fdfbfb, #ebf1d1) !important; /* Krem lembut */
					...
					<div className="mb-3">
						<Form.Group className="mb-3">
							...
							<div className="d-flex">
								...
								/>
								<Button variant="outline-primary" onClick={handleScanQR} className="d-flex align-items-center ms-2">
									<QrCode size={16} />
								</Button>
							</div>
						</Form.Group>
